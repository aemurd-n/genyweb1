  ...
  if (!service) {
    return (
      <Container sx={{ py: 10, textAlign: "center" }}>
        <MDTypography variant="h4">{t('common:common.serviceNotFound')}</MDTypography>
      </Container>
    );
  }

  const Icon = service.icon;
  const translationKey = service.translationKey;
      ...
      <Container maxWidth="lg">
        ...
        <MDBox sx={{ mb: 6, textAlign: "justify" }}>
          <MDTypography variant="body1" sx={{ lineHeight: 1.8, whiteSpace: "pre-wrap", mb: 4 }}>
            {t(`services:${translationKey}.fullDescription`)}
          </MDTypography>
        </MDBox>
        ...
        <MDBox sx={{ mb: 6 }}>
          <MDTypography variant="h4" sx={{ mb: 4, fontWeight: "bold" }}>
            {t('common:common.useCases')}
          </MDTypography>
          <Grid container spacing={3}>
            {t(`services:${translationKey}.useCases`, { returnObjects: true }).map((useCase, index) => (
        ...
        <MDBox sx={{ mb: 6 }}>
          <MDTypography variant="h4" sx={{ mb: 4, fontWeight: "bold" }}>
            {t('common:common.keyBenefits')}
          </MDTypography>
          <Grid container spacing={2}>
            {t(`services:${translationKey}.benefits`, { returnObjects: true }).map((benefit, index) => (
              <Grid item xs={12} sm={6} key={index}>
